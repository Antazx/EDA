...
public class DesOfuscador {
	...
	public short[] algoritmo(short[] txt, int clave) {
	 ...
	 */
	public ArrayList<Short> leer(String fichero) {
	...
	public int buscar(short[] leido, short[] cadena){

        int i=0, j, x, contador=0, posicion, pos=-1;
        ...
        if(coincidencias.length>0){
            ...
            for (i=0; i<coincidencias.length; i++){
                    ...
                    if (contador == cadena.length-1) {
                    	...
                    	//falla esto porque vete tu a saber
                        System.out.println("Acierto " +posicion);
                        return posicion;
	...
	/**
	 * Busca las posiciones en las que hay la primera letra de la cadena a
	 * buscar
	 * 
	 * @param leido
	 *            texto en el que vamos a buscar
	 * @param posicion
	 *            de la cadena que vamos a buscar
	 * @return ArrayList de enteros con las posiciones que ha encontrado
	 */
	public ArrayList<Integer> coincidencias(short[] leido, short posicion) {
	...
	public void imprimir(short[] txt, int ini, int fin) {

		char c;
		short s;
		if (ini + fin <= (txt.length) && ini >= 0) {
			...
			System.out.println("Metodo imprimir: No se puede acceder a la posicion del array");
		}

	}

	public short[] cortar(short[] array, int ini, int tamaño) {
		short[] cortado = new short[tamaño + 596];
		for (int i = 0; i < cortado.length; i++) {
			cortado[i] = array[i + ini];
		}
